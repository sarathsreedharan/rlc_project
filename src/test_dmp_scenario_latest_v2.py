...
if __name__ == '__main__':
    ...
    return_dmp_record_file = 'record_files_test/record_file_new_return'
    move_dmp_record_file = 'record_files_test/record_file_new_move_fwd'    

    lateral_magic_numbers = [-0.026, -0.1, 0.048] #[-0.009, -0.075, 0.059] #[-0.043, -0.074, 0.039] #[-0.020, -0.071, 0.053] #[-0.015, -0.072, 0.037]
    vertical_magic_numbers = [0.004, -0.005, 0.063]#[0.004, -0.005, 0.039] #[-0.019, -0.055, 0.093]
    drop_magic_numbers = [-0.029, -0.087, 0.238]
#    drop_magic_numbers = [-0.0, -0.00, 0.15]
    pour_magic_numbers = [0.029, -0.163, 0.228] #[0.0, -0.0, 0.15] #[0.029, -0.163, 0.148] #[0.026, 0.158, 0.132] #[0.026, -0.170, 0.139]
...
#    ice_cube = list(ice_cube_pos) + list(initial_orient)
    ...
    traj = []
    with open(return_dmp_record_file) as tr_fd:
        for line in tr_fd:
             traj.append([float(i) for i in line.strip().split(',')])
    ...
    curr = list(curr_pos) + list(curr_orient)
#    ice_cube = list(ice_cube_pos) + list(initial_orient)
    init_thresh = [0,0,0,0,0,0,0]
    ...
    integrate_iter = 5       #dt is rather large, so this is > 1 
    print ("starting planning")
    plan = makePlanRequest(curr, init_dot, t_0, init, init_thresh,
                           seg_length, tau, dt, integrate_iter)
    print ("planning completed")
    waypoints = []
    curr_pose = group.get_current_pose().pose
   ...
   # 2.b Move to glass
    ...
    traj = []
    with open(move_dmp_record_file) as tr_fd:
        for line in tr_fd:
             traj.append([float(i) for i in line.strip().split(',')])
...
#    ice_cube = list(ice_cube_pos) + list(initial_orient)
    ...
    glass_goal = list(glass_pos) + list(curr_orient)
    print ("starting planning")
    plan = makePlanRequest(curr, init_dot, t_0, glass_goal, init_thresh,
                           seg_length, tau, dt, integrate_iter)
    print ("planning completed")
    waypoints = []
    curr_pose = group.get_current_pose().pose
    ...
    for point in plan.plan.points:
        ...
        tmp_pose.orientation.z = step[5]
        tmp_pose.orientation.w = step[6]
        tmp_pose.position.x = step[0] + drop_magic_numbers[0]
        tmp_pose.position.y = step[1] + drop_magic_numbers[1]
        tmp_pose.position.z = step[2] + drop_magic_numbers[2]
        waypoints.append(copy.deepcopy(tmp_pose))
...
#    ice_cube = list(ice_cube_pos) + list(initial_orient)
    ...
    for point in plan.plan.points:
        ...
        tmp_pose.orientation.z = step[5]
        tmp_pose.orientation.w = step[6]
        tmp_pose.position.x = step[0] + pour_magic_numbers[0]
        tmp_pose.position.y = step[1] + pour_magic_numbers[1]
        tmp_pose.position.z = step[2] + pour_magic_numbers[2]
        waypoints.append(copy.deepcopy(tmp_pose))
